...
class CurrentsUpdater(Updater):
    ...
    def plot(self):
        ...
        u_points = u_raw[valid] * 1.94384  # Convert m/s to Knots
        v_points = v_raw[valid] * 1.94384
        ...
        logger.debug(
            ...
            f"Width Factor: {width_factor}x | Thickness Range: {min_thickness * width_factor:.1f} to {max_thickness * width_factor:.1f}"
        )
    ...
    def run(self):
        ...
        try:
            ...
            if needs_download:
                logger.info(f"Downloading fresh RTOFS currents data from {url}...")
